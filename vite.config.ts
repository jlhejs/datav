...
export default (cenv: ConfigEnv) => {
  const { mode, command } = cenv
  const dirRoot = process.cwd()

  const env = loadEnv(mode, dirRoot)
  let setting  ={}
  if (env.VITE_IS_COMPONENT === 'true') {
    setting = buildPackage(cenv)
  }else{
    setting = {
      ...
      resolve: {
        alias: {
          '@': pathResolve('./src'),
          'datav-vue': pathResolve('./package/datav-vue/index.ts'),
