import { defineComponent, ref, computed, toRef, watchEffect, openBlock, createElementBlock, normalizeStyle, createElementVNode, toDisplayString, Fragment, renderList } from "vue";
import { useApiStore, useDataCenter, getFieldMap, Accounting } from "datav-vue";
import "shortid";
...
var _addComplexStringPropTween = function _addComplexStringPropTween2(target, prop, start, end, setter, stringFilter, funcParam) {
  ...
  if (!tl || keyframes && !vars.stagger) {
    ...
    if (startAt) {
      ...
      time < 0 && (_reverting$1 || !immediateRender && !autoRevert) && tween._startAt.revert(_revertConfig);
      if (immediateRender) {
        if (dur && time <= 0) {
...
const _sfc_main = defineComponent({
  ...
  setup(props) {
    ...
    const numVal = ref(0);
    const config3 = toRef(props.com, "config");
    const attr = toRef(props.com, "attr");
    const titleText = computed(() => {
      return dv_data.value[dv_field.value.title] || config3.value.title.content;
    });
    const prefixText = computed(() => {
      var _a;
      return (_a = dv_data.value[dv_field.value.prefix]) != null ? _a : config3.value.counter.prefix.content;
    });
    const suffixText = computed(() => {
      var _a;
      return (_a = dv_data.value[dv_field.value.suffix]) != null ? _a : config3.value.counter.suffix.content;
    });
    const separatingSymbol = computed(() => {
      return config3.value.numbers.separatingSymbol ? config3.value.numbers.separatingSymbol.charAt(0) : ",";
    });
    const decimalSymbol = computed(() => {
      return config3.value.numbers.decimalSymbol ? config3.value.numbers.decimalSymbol.charAt(0) : ".";
    });
    const duration = computed(() => {
      return config3.value.numbers.duration > 0 ? config3.value.numbers.duration : 0;
    });
    const realNumber = computed(() => {
      const { numbers } = config3.value;
    ...
    const wrapperStyle = computed(() => {
      const arrangement = config3.value.global.arrangement;
      let style = {};
      if (arrangement === "top") {
        style = {
          display: "block",
    ...
    const titleStyle = computed(() => {
      const { fontFamily, arrangement, distance } = config3.value.global;
      const { textStyle } = config3.value.title;
      const style = {
        display: "flex",
        alignItems: "center",
    ...
    const counterStyle = computed(() => {
      const { title, counter, numbers, global } = config3.value;
      const style = {
        display: "flex",
        alignItems: "baseline",
    ...
    const prefixStyle = computed(() => {
      const { prefix, margin } = config3.value.counter;
      return {
        ...
        VerticalAlign: "super",
        fontFamily: config3.value.global.fontFamily
      };
    });
    const suffixStyle = computed(() => {
      const { suffix, margin } = config3.value.counter;
      return {
    ...
    watchEffect(() => {
      const { numbers } = config3.value;
      let num = dv_data.value[dv_field.value.value] || 0;
      const divisor = numbers.divisor || 0;
      if (divisor !== 0)
        num = index.divide(num, divisor);
      if (numbers.animation) {
        gsapWithCSS.to(numVal, {
          duration: duration.value / 1e3,
          value: num
        });
      } else {
        numVal.value = num;
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createElementBlock("div", {
    ...
    createElementVNode("div", {
      style: normalizeStyle(_ctx.counterStyle)
    }, [
      createElementVNode("span", {
        style: normalizeStyle(_ctx.prefixStyle)
      }, toDisplayString(_ctx.prefixText), 5),
      (openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.realNumber, (n, i) => {
        return openBlock(), createElementBlock("span", {
          key: i,
          style: normalizeStyle([n === _ctx.separatingSymbol ? _ctx.separateCharStyle : _ctx.numberStyle])
        }, toDisplayString(n), 5);
      }), 128)),
      createElementVNode("span", {
        style: normalizeStyle(_ctx.suffixStyle)
      }, toDisplayString(_ctx.suffixText), 5)
    ], 4)
  ], 4);
}
var NumberTitleFlop = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__file", "F:/datav-jlhe/src/components/text/number-title-flop/src/index.vue"]]);
