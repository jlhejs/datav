<template>
  <div :class="['g-aside config-panel-wp', { '--hide': !visiblePanel }]">
    <div class="config-manager">
      <layer-setting-panel v-if="com && com.type === ComType.layer" />
      <n-tabs v-else-if="com" type="card">
...
import { useToolbarStore,useModulesStore, modules } from 'datav-vue'
import { useComStore } from '@/store/com'
import { loadAsyncComponent } from '@/utils/async-component'
import { IconSetting, IconCloud, IconInteract } from '@/icons'
import { comInjectionKey, configInjectionKey } from './config'
...
export default defineComponent({
  ...
  setup() {
    ...
    const com = computed(() => comStore.selectedCom)
    const selectedCount = computed(() => comStore.selectedComs.length)
    const config = computed(() => modulesStore.config)
